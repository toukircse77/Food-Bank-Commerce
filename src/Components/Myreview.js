...
const Myreview = ({myview,user}) => {
	...
	<div className="flex flex-col space-y-4 md:space-y-0 md:space-x-6 md:flex-row">
		...
		<div className="flex flex-col">
			<h4 className="text-lg font-semibold text-center md:text-left">{myview.reviewName}</h4>
			<p className="dark:text-gray-400">My review: {myview.message}</p><br />
            <span>{myview?.email}</span>
		</div>
	</div>
</div>
        </div>
